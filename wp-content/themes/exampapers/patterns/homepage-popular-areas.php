...
<section class="exampapers-page-section exampapers-section-white">
	<div class="exampapers-section-inner">
		...
		<nav class="exampapers-feature-grid" aria-label="<?php esc_attr_e( 'Popular 11+ areas', 'exampapers' ); ?>">
			<?php foreach ( $area_terms as $term ) : ?>
				...
				$excerpt = trim( wp_strip_all_tags( term_description( $term, 'product_cat' ) ) );

				if ( '' === $excerpt ) {
					$excerpt = sprintf(
						/* translators: %s: exam area name. */
						__( 'Practice papers for %s preparation.', 'exampapers' ),
						$term->name
					);
				}
				?>
				<a class="exampapers-card exampapers-nav-card" href="<?php echo esc_url( $link ); ?>">
					...
					<small><?php echo esc_html( wp_trim_words( $excerpt, 12 ) ); ?></small>
				</a>
			<?php endforeach; ?>
		</nav>
